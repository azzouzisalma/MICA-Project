...
figure
plot(auto_cov)
xlabel('k'),title('auto_covariance')

GAMMAK=xcov(diff(R_pos));
figure
plot(GAMMAK)
title('auto_covariance with xcov')
...
for i=1:length(BPM)-1                                                                        
    if(BPM(i)>240 && BPM(i)<600)
        a=index(2*i-1);             %a is time of the beginning of first PQRST complex
        b=index(2*i);              %b is time of the end of first PQRST complex
        c=index(2*i+1);             %c is time of the beginning of the next PQRST complex
        d=index(2*i+2);             %d is time of the last of the next PQRST complex
        p1=P_pos(i-1);              %P position associated to i� PQRST complex
        p2=P_pos(i);
        q1=Q_pos(i+1);
        q2=Q_pos(i+2);
        r1=R_pos(i);
        ...
        t1=T_pos(i);
        t2=T_pos(i+1);
        if((p1<a || p1>b)&&(r1<a || r1>b)&&(q1<a || q1>b)&&(s1<a ||s1>b)&&(t1<a || t1>b)&&(p2<c || p2>d)&&(q2<c || q2>d)&&(r2<c || r2>d)&&(s2<c || s2>d)&&(t2<c || t2>d))
            index_VF=[index_VF i];
            VF=1;
        end
    end
